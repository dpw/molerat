...
HDRS=base.h buffer.h thread.h tasklet.h application.h queue.h poll.h watched_fd.h stream.h socket.h echo_server.h http-parser/http_parser.h http_reader.h http_status.h http_writer.h http_server.h skinny-mutex/skinny_mutex.h

$(foreach H,$(HDRS),$(eval HDROBJS_$(SRCPATH)$(H)=$(H:%.h=%.o)))
HDROBJS_$(SRCPATH)stream.h=
HDROBJS_$(SRCPATH)poll.h=poll_common.o

ifdef USE_EPOLL
HDROBJS_$(SRCPATH)watched_fd.h=poll_epoll.o
HDROBJS_$(SRCPATH)timer.h=poll_epoll.o
else
HDROBJS_$(SRCPATH)watched_fd.h=poll_poll.o
HDROBJS_$(SRCPATH)timer.h=poll_poll.o
endif
